...
{
	/// <summary>
	/// Partial class containing the PostgreSQL SQL generation logic for the EQL engine.
	/// <para>
	/// Migrated from the monolith's <c>WebVella.Erp.Eql.EqlBuilder.Sql.cs</c> (960 lines) with
	/// namespace updates for the SharedKernel. All SQL generation patterns — parameterized query
	/// construction, relation join generation, correlated subqueries for 1:1/1:N/M:N relations,
	/// WHERE clause processing, ORDER BY, LIMIT/OFFSET pagination, FTS (full-text search) via
	/// <c>to_tsvector/to_tsquery</c>, and CONTAINS/STARTSWITH operators — are preserved identically
	/// to produce byte-for-byte equivalent SQL output.
	/// </para>
	/// <para>
