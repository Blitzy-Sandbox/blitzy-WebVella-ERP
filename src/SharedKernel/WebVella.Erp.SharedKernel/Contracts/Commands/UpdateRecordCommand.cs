...
namespace WebVella.Erp.SharedKernel.Contracts.Commands
{
	...
	public class UpdateRecordCommand : ICommand
	{
		...
		/// <summary>
		/// The unique identifier of the user who initiated the update operation.
		/// <para>
		/// In the monolith, <c>RecordManager.UpdateRecord</c> obtains the user identity from
		/// <c>SecurityContext.CurrentUser</c>. In the microservice architecture, the user identity
		/// must be explicitly propagated with each command since the consuming service cannot access
		/// the originating service's security context. The API Gateway or originating service
		/// populates this from the authenticated JWT claims before dispatching the command.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// The roles assigned to the user who initiated the update operation.
		/// <para>
