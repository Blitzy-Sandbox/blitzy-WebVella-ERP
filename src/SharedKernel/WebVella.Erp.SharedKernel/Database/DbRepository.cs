...
{
	/// <summary>
	/// Provides ambient access to the current IDbContext instance.
	/// Each microservice registers its own IDbContext implementation (CoreDbContext, CrmDbContext, etc.)
	/// during startup/DI configuration. DbRepository accesses connections via DbContextAccessor.Current.CreateConnection().
	/// Uses AsyncLocal&lt;T&gt; for async-safe per-execution-context scoping.
	/// </summary>
	public static class DbContextAccessor
	{
		private static readonly AsyncLocal<IDbContext> _current = new AsyncLocal<IDbContext>();

		/// <summary>
		/// Gets or sets the current IDbContext for the async execution context.
		/// Throws InvalidOperationException if accessed when no context has been set.
		/// </summary>
		public static IDbContext Current
		{
			get => _current.Value ?? throw new InvalidOperationException("No DbContext is available. Call CreateContext first.");
			set => _current.Value = value;
		}
	}

	/// <summary>
	/// Static PostgreSQL DDL/DML helper providing shared schema and record operations
	/// used by ALL microservice database layers. All SQL generation patterns, type mappings,
	/// and default value conversions produce identical SQL to the original monolith.
	/// Connection access uses DbContextAccessor.Current.CreateConnection() for microservice compatibility.
	/// </summary>
