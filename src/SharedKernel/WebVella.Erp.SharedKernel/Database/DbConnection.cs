...
{
	/// <summary>
	/// Abstraction for database context coordination.
	/// Each microservice implements this interface in its service-specific DbContext.
	/// Provides connection creation, transactional state management, and connection lifecycle.
	/// </summary>
	public interface IDbContext : IDisposable
	{
		/// <summary>Creates a new DbConnection, optionally sharing the current transaction.</summary>
		DbConnection CreateConnection();

		/// <summary>Closes a DbConnection and removes it from the context's connection stack.</summary>
		bool CloseConnection(DbConnection conn);

		/// <summary>Enters transactional state — sets the shared transaction for new connections.</summary>
		void EnterTransactionalState(NpgsqlTransaction transaction);

		/// <summary>Leaves transactional state — clears the shared transaction.</summary>
		void LeaveTransactionalState();
	}
	...
	public class DbConnection : IDisposable
	{
		...
		private IDbContext CurrentContext;

		/// <summary>
		/// 
		/// </summary>
		/// <param name="transaction"></param>
		internal DbConnection(NpgsqlTransaction transaction, IDbContext suppliedContext)
		{
		...
		internal DbConnection(string connectionString, IDbContext suppliedContext)
		{
			CurrentContext = suppliedContext;
			transaction = null;
