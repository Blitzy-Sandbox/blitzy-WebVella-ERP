...
{
	/// <summary>
	/// Core service dynamic-record CRUD and query engine for the Core Platform microservice.
	/// Responsible for translating EntityQuery/QueryObject filters into PostgreSQL SQL,
	/// supporting relational projections via JSON aggregation, handling geography fields
	/// with PostGIS transforms, and managing record lifecycle operations (Create, Update, Delete, Find, Count).
	///
	/// Adapted from WebVella.Erp/Database/DbRecordRepository.cs (2097 lines).
	///
	...
	///   - Imports updated to SharedKernel + Service.Core types
	///   - RelationFieldMeta properties cast to EntityRelation/Entity where needed (SharedKernel uses object types)
	///   - All SQL generation, CRUD logic, and query engine preserved byte-for-byte
	///
	/// NOTE: Cross-service EQL queries (referencing entities in other service databases) are handled
	/// at the EQL builder level, NOT in this repository. This repository only executes SQL against
	/// the Core service's own database.
	/// </summary>
	public class DbRecordRepository
	{
		...
		private EntityRelationManager _relMan;
		FtsAnalyzer ftsAnalyzer = new FtsAnalyzer();
		private CoreDbContext suppliedContext = null;

		/// <summary>
		/// Lazy-initialized EntityManager. Defers creation until first use to avoid
		/// ArgumentNullException when ErpSettings.Configuration is not yet initialized
		/// (e.g., during CoreDbContext construction in unit tests).
		/// </summary>
		private EntityManager entMan
		{
			...
			{
				if (_entMan == null)
					_entMan = new EntityManager(CurrentContext, ErpSettings.Configuration);
				return _entMan;
			}
		}

		/// <summary>
		/// Lazy-initialized EntityRelationManager. Defers creation until first use to avoid
		/// ArgumentNullException when ErpSettings.Configuration is not yet initialized
		/// (e.g., during CoreDbContext construction in unit tests).
		/// </summary>
		private EntityRelationManager relMan
		{
			...
			{
				if (_relMan == null)
					_relMan = new EntityRelationManager(CurrentContext, ErpSettings.Configuration);
				return _relMan;
			}
		}
        ...
        public EntityRecord ConvertJObjectToEntityRecord(JObject jObj, List<Field> fields)
        {
            EntityRecord record = new EntityRecord();
        ...
        public List<EntityRecord> Find(EntityQuery query)
        {
            ...
            if (noSelectRelations)
            {
				...
				using (var conn = CurrentContext.CreateConnection())
                {
            ...
            else
            {
                ...
                foreach (var field in fields)
                {
                    ...
                    else
                    {
                        ...
                        var relation = (EntityRelation)relationField.Relation;
                        ...
                        if (relation.RelationType == EntityRelationType.OneToOne)
                        {
                            ...
                            if (relation.OriginEntityId == entity.Id)
                            {
                                ...
                                sql.AppendLine(string.Format(OTM_RELATION_TEMPLATE,
                                    ...
                                    GetTableNameForEntity((Entity)relationField.TargetEntity),
                                    relationName,
                                    relationField.TargetField.Name,
                                    GetTableNameForEntity((Entity)relationField.OriginEntity),
                                        ...
                                        List<KeyValuePair<string, string>> overwriteArgs = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(query.FieldName))
            {
                ...
                else
                {
                    ...
                    var rel = (EntityRelation)relationFieldMeta.Relation;

                    if (rel.TargetEntityId != entity.Id && rel.OriginEntityId != entity.Id)
                    ...
                    //Entity entity = entMan.ReadEntity(query.EntityName).Object;
                    relationFieldMeta.TargetEntity = entMan.ReadEntity(rel.TargetEntityId).Object;
                    relationFieldMeta.OriginEntity = entMan.ReadEntity(rel.OriginEntityId).Object;

                    var targetEntity = (Entity)relationFieldMeta.TargetEntity;
                    var originEntity = (Entity)relationFieldMeta.OriginEntity;
                    ...
                    relationFieldMeta.TargetField = targetEntity.Fields.Single(x => x.Id == rel.TargetFieldId);
                    relationFieldMeta.OriginField = originEntity.Fields.Single(x => x.Id == rel.OriginFieldId);
                    ...
                    if (targetEntity.Id == entity.Id)
                        joinToEntity = originEntity;
                    else
                        joinToEntity = targetEntity;
                    ...
                    if (rel.RelationType == EntityRelationType.OneToOne)
                    {
                        //when the relation is origin -> target entity
                        if (rel.OriginEntityId == entity.Id)
                        {
                            relationJoinSql = string.Format(FILTER_JOIN,
                                GetTableNameForEntity(targetEntity), relationName,
                                relationName, relationFieldMeta.TargetField.Name,
                                GetTableNameForEntity(originEntity), relationFieldMeta.OriginField.Name);
                        }
                        else //when the relation is target -> origin, we have to query origin entity
                        {
                            relationJoinSql = string.Format(FILTER_JOIN,
                                   GetTableNameForEntity(originEntity), relationName,
                                   relationName, relationFieldMeta.OriginField.Name,
                                   GetTableNameForEntity(targetEntity), relationFieldMeta.TargetField.Name);
                        }
                    }
                    else if (rel.RelationType == EntityRelationType.OneToMany)
                    {
                        //when origin and target entity are different, then direction don't matter
                        if (rel.OriginEntityId != rel.TargetEntityId)
                        {
                            //when the relation is origin -> target entity
                            if (rel.OriginEntityId == entity.Id)
                            {
                    ...
                    else if (rel.RelationType == EntityRelationType.ManyToMany)
                    {
                        string relationTable = "rel_" + rel.Name;
                        string targetJoinAlias = relationName + "_target";
                        string originJoinAlias = relationName + "_origin";
                        string targetJoinTable = GetTableNameForEntity(targetEntity);
                        string originJoinTable = GetTableNameForEntity(originEntity);
        ...
        public string GetTableNameForEntity(Entity entity)
        {
            return GetTableNameForEntity(entity.Name);
        }

        public string GetTableNameForEntity(string entityName)
        {
        ...
        public object ExtractQueryFieldJsonValue(string value, Field field, List<KeyValuePair<string, string>> overwriteArgs, out bool skipClause)
        {
            skipClause = false;
            JObject jObj = null;
            try
        ...
        public dynamic ExtractSortFieldJsonValue(string value, List<KeyValuePair<string, string>> overwriteArgs)
        {
            JObject jObj = null;
            try
            {
