...
{
	/// <summary>
	/// Stores relation metadata as JSON documents in the entity_relations table,
	/// synchronizes FK constraints or many-to-many rel_* join tables, maintains
	/// relation-specific indexes, and provides lightweight rel-table row maintenance
	/// (CreateManyToManyRecord / DeleteManyToManyRecord).
	///
	/// Scoped to Core-only relations within the erp_core database.
	///
	/// Adapted from the monolith's WebVella.Erp.Database.DbRelationRepository:
	///   - Namespace changed: WebVella.Erp.Database → WebVella.Erp.Service.Core.Database
	///   - DbContext replaced with CoreDbContext (service-scoped ambient context)
	///   - All DbContext.Current.CreateConnection() calls replaced with CurrentContext.CreateConnection()
	///   - Imports updated to SharedKernel types (DbRepository, DbConnection, DbParameter, etc.)
	///   - All business logic, SQL strings, exception messages, and Cache.Clear() patterns preserved exactly.
	/// </summary>
	public class DbRelationRepository
	{
		...
		public DbRelationRepository(CoreDbContext currentContext)
		{
