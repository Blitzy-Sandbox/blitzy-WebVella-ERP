...
{
    #region << Service proxy interfaces for cross-service communication >>

    /// <summary>
    /// Abstraction over entity repository operations (monolith's DbContext.Current.EntityRepository).
    /// </summary>
    ...
    /// <summary>
    /// Abstraction over relation repository operations (monolith's DbContext.Current.RelationRepository / EntityRelationManager).
    /// </summary>
    public interface IRelationRepository
    {
        List<DbEntityRelation> Read();
        EntityRelation ReadRelation(Guid id);
    }

    /// <summary>
    /// Abstraction over record query operations (monolith's DbContext.Current.RecordRepository / RecordManager).
    /// </summary>
    public interface IRecordRepository
    {
        List<EntityRecord> Find(EntityQuery query);
    }

    /// <summary>
    /// Abstraction for application service operations (monolith's AppService).
    /// </summary>
    public interface IAppServiceClient
    {
        List<App> GetAllApplications(bool useCache = true);
        List<App> GetAllApplications(string connectionString, bool useCache = false);
    }

    /// <summary>
    /// Abstraction for page service operations (monolith's PageService).
    /// </summary>
    public interface IPageServiceClient
    {
        List<ErpPage> GetAll(bool useCache = true);
        List<ErpPage> GetAll(string connectionString, bool useCache = false);
        List<PageBodyNode> GetAllBodyNodes();
        List<PageBodyNode> GetAllBodyNodes(string connectionString);
        List<PageBodyNode> GetPageNodes(Guid pageId);
        List<PageBodyNode> GetPageNodes(string connectionString, Guid pageId);
    }

    #endregion

    #region << Local DTO types for web-layer models not in SharedKernel >>

    /// <summary>
    /// Minimal App model (was WebVella.Erp.Web.Models.App).
    /// Retained as a local DTO because the full App type is not in SharedKernel.
    /// Sitemap, SitemapArea, SitemapGroup, SitemapNode, and ErpPage types are
    /// defined in SharedKernel.Models (NavigationModels.cs) and AdminServiceInterfaces.cs.
    /// </summary>
    public class App
    {
    ...
    /// <summary>Page body node model (was WebVella.Erp.Web.Models.PageBodyNode).</summary>
    ...
    /// <summary>Page data source model (was WebVella.Erp.Web.Models.PageDataSource).</summary>
    ...
    /// <summary>
    /// Interface for the code generation service, enabling DI registration and testability.
    /// </summary>
    public interface ICodeGenService
    {
        MetaChangeResponseModel EvaluateMetaChanges(
            string connectionString,
            List<string> entityRecordsToCompare,
            bool includeEntityMeta,
            bool includeEntityRelations,
            bool includeRoles,
    ...
    /// <summary>
    /// Diff-based C# migration code generator for the Admin microservice.
    /// Migrated from WebVella.Erp.Plugins.SDK.Services.CodeGenService with DI refactoring.
    /// All business logic is preserved exactly as in the monolith.
    /// </summary>
    public class CodeGenService : ICodeGenService
    {
        ...
        private readonly IPageServiceClient _pageServiceClient;
        ...
        /// <summary>
        /// Initializes a new CodeGenService with dependency-injected services replacing
        /// the monolith's direct instantiation pattern.
        /// </summary>
        public CodeGenService(
            ...
            IPageServiceClient pageServiceClient,
            ...
            string defaultCulture = "en-US")
        {
            ...
            _pageServiceClient = pageServiceClient ?? throw new ArgumentNullException(nameof(pageServiceClient));
            _adminDbContext = adminDbContext ?? throw new ArgumentNullException(nameof(adminDbContext));
                    ...
                    List<string> NNRelationsRecordsToCompare)
        {
            ...
            #region << Get elements >>
            currentEntityList = _entityRepository.Read();
            currentRelationsList = _relationRepository.Read();
            currentRoleList = _recordRepository.Find(queryRole);
            ...
            if (includeApplications)
            {
                ...
                var currentPages = _pageServiceClient.GetAll();
                ...
                var currentApps = _appServiceClient.GetAllApplications();
            ...
            if (entityRecordsToCompare != null && entityRecordsToCompare.Count > 0)
            {
                ...
                foreach (var id in entityRecordsToCompare)
                {
                    ...
                    {
                        DbEntity currentEntity = currentEntityList.First(x => x.Id == entityId);
                        var currentRecords = _recordRepository.Find(new EntityQuery(currentEntity.Name));
            ...
            if (NNRelationsRecordsToCompare != null && NNRelationsRecordsToCompare.Count > 0)
            {
                ...
                foreach (var id in NNRelationsRecordsToCompare)
                {
                    ...
                    var relation = _relationRepository.ReadRelation(relationId);
                    if (relation == null)
        ...
        {
            return _appServiceClient.GetAllApplications(OldDbConnectionString, useCache: false);
        }

        private List<ErpPage> ReadOldPages()
        {
            return _pageServiceClient.GetAll(OldDbConnectionString, useCache: false);
        }

        private List<PageBodyNode> ReadOldPageBodyNodes(Guid pageId)
        {
            return _pageServiceClient.GetPageNodes(OldDbConnectionString, pageId);
        }

        private List<SitemapArea> ReadOldSitemapAreas()
        {
            var apps = _appServiceClient.GetAllApplications(OldDbConnectionString, useCache: false);
        ...
        {
            var apps = _appServiceClient.GetAllApplications();
            List<SitemapArea> areas = new List<SitemapArea>();
            foreach (var app in apps)
            {
                if (app.Sitemap != null && app.Sitemap.Areas != null)
        ...
        {
            return _pageServiceClient.GetAllBodyNodes(OldDbConnectionString);
        }

        private List<PageBodyNode> ReadCurrentPageBodyNodes()
        {
            return _pageServiceClient.GetAllBodyNodes();
        }
...
"{\n" +
    ...
    $"\ttreeSelectField.Label = \"{currentField.Label}\";\n";
        ...
        {
            var allEntities = _entityRepository.Read();
            var originEntity = allEntities.Single(x => x.Id == relationRecord.OriginEntityId);
            var originField = originEntity.Fields.Single(x => x.Id == relationRecord.OriginFieldId);
            var targetEntity = allEntities.Single(x => x.Id == relationRecord.TargetEntityId);
        ...
        private UpdateCheckResponse UpdateRelationCode(DbEntityRelation currentRelation, DbEntityRelation oldRelation)
        {
            var response = new UpdateCheckResponse();
            var allEntities = _entityRepository.Read();
            var originEntity = allEntities.Single(x => x.Id == currentRelation.OriginEntityId);
            var originField = originEntity.Fields.Single(x => x.Id == currentRelation.OriginFieldId);
            var targetEntity = allEntities.Single(x => x.Id == currentRelation.TargetEntityId);
...
"{\n" +
    ...
    $"\tpatchObject[\"id\"] = new Guid(\"{(Guid)currentRole["id"]}\");\n";
        ...
        private void CompareEntityRecords(DbEntity oldEntity, DbEntity currentEntity, List<EntityRecord> recordsToCreate, List<EntityRecord> recordsToUpdate, List<EntityRecord> recordsToDelete)
        {
            ...
            var currentRecords = _recordRepository.Find(new EntityQuery(currentEntity.Name));
            var currentRecordsDict = currentRecords.AsRecordDictionary();
