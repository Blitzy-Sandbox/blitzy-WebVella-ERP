...
| `openapi-lint` | **Committed docs CI (guarded)** | Spectral (`@stoplight/spectral-cli@6.16.2`) | `spectral lint openapi/v1.json` | Runs only when `openapi/v1.json` exists; otherwise emits a `::notice::` and skips gracefully (rule F). The document is produced later by `WebVella.Erp.Api`. Source: /.github/workflows/docs.yml |
