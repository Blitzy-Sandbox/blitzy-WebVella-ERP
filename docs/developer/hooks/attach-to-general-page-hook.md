...
| name | description |
|------|------|
| `key` | *object type*: `string`<br>*default value*: `string.Empty`<br>*is required*: `FALSE`<br>The name of a custom hook handler -> if the page has a queryParam `hookKey`, only hooks that match it with their `key` will be executed |
