...
The background job type is defined by the `ErpJob` object. This object works with close relation with the `SchedulePlan` object and when scheduled it creates a number of background jobs which execute the same method, as assigned in the type. Today these job types execute inside the web application host; a migration to a dedicated background worker host (`WebVella.Erp.Worker`) is planned but not yet implemented, and the `ErpJob` contract below is unchanged either way.
