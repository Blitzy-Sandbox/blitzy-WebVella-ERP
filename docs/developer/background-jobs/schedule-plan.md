...
In order for a background job to be scheduled by the platform, there should be a registered schedule plan which manages the process. All schedule plans of a plugin should be checked for existence and registered during your plugin's `Initialize` method in the `ErpPlugin`-inherited class. Once registered the schedule plan will be executed by the system, even if removed from the plugin initialization. (Under the planned `IErpPlugin` contract — not yet implemented — this registration would move to the plugin's `OnLoadAsync`/`OnMigrateAsync` lifecycle methods.)
...
You can register a schedule plan by executing the following code in your plugin's `Initialize` method (the planned `IErpPlugin` equivalent would be `OnLoadAsync`/`OnMigrateAsync`):
