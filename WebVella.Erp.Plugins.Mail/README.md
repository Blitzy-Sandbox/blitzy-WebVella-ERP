...
Host-level email settings bind from the `Settings` section of the host configuration and are exposed through `ErpSettings`. `Source: /WebVella.Erp/ErpSettings.cs:L29-L35,L99-L105` They are documented below **by key name and default only** — never place real credentials in documentation or configuration files.

| Key | Purpose | Default |
|-----|---------|---------|
| `EmailEnabled` | Master on/off switch for email sending | `false` |
...
Secrets (`EmailSMTPUsername`, `EmailSMTPPassword`) must be supplied through environment variables or Kubernetes Secrets and must never be committed to source control.
...
| Nothing sends at all | `EmailEnabled` is `false` (the default), or the target `smtp_service` has `is_enabled = false` | Enable email and enable the target SMTP service. |
| Queue never drains | The queue only drains when the schedule plan runs `ProcessSmtpQueueJob` (~every 10 minutes). `Source: /WebVella.Erp.Plugins.Mail/MailPlugin.cs:L41-L82` | Ensure the scheduler is running; under the headless refactor this runs in the `WebVella.Erp.Worker` host — confirm the worker is up. |
