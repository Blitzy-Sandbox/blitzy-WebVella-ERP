...
| `Settings__Serilog__MinimumLevel` | Minimum level for structured JSON logging (for example `Information` or `Warning`). *Proposed — Not available / to be confirmed.* See [Observability](../docs/architecture/observability.md) and the canonical [configuration reference](../docs/deployment/configuration-reference.md#observability-proposed). | `Not available / to be confirmed` |
| `Settings__Otlp__Endpoint` | OTLP exporter endpoint URL for trace/metric export. Must **not** embed credentials; collector auth is a separate Secret. *Proposed — Not available / to be confirmed.* See [Observability](../docs/architecture/observability.md) and the canonical [configuration reference](../docs/deployment/configuration-reference.md#observability-proposed). | `Not available / to be confirmed` |
