	...
	public static class CodeEvalService
	{
		...
		private static ICodeVariable GetScriptObject(string sourceCode)
		{
			...
			lock (lockObj)
			{
				...
				// SECURITY (A03 / CWE-94, ACCEPTED RISK): this compiles & executes admin-authored C# at runtime. It is a
				// deliberate, trusted-author feature (code data sources, page-component code, snippets). Only administrators
				// can AUTHOR/persist the code that reaches this method: authorship is gated by class-level [Authorize] plus
				// [Authorize(Roles = "administrator")] on the persistence endpoints in WebVella.Erp.Web.Controllers.WebApiController
				// and by the admin-only page-builder/SDK UI. Do NOT expose this to untrusted input. Documented as accepted-risk in SECURITY.md.
