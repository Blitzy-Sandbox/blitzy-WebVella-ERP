...
SEVERITY: Critical
CWE: CWE-502
LOCATION: WebVella.Erp/Jobs/JobDataService.cs (TypeNameHandling.All); WebVella.Erp/Notifications/NotificationContext.cs; WebVella.Erp/Database/DbEntityRepository.cs; WebVella.Erp/Database/DbRelationRepository.cs (TypeNameHandling.Auto); WebVella.Erp/Api/Models/AutoMapper/Profiles/JobProfile.cs (TypeNameHandling.All); WebVella.Erp.Plugins.SDK/Services/CodeGenService.cs (TypeNameHandling.Auto — the ReadOldEntities/ReadOldRelations legacy-migration readers)
...
REMEDIATION: Set TypeNameHandling.None where polymorphism is unnecessary; where polymorphic round-tripping of persisted data is genuinely required, attached a new fail-closed allowlist WebVella.Erp/Utilities/ErpSerializationBinder.cs (ISerializationBinder) that resolves only an explicitly permitted set of types and THROWS for everything else, neutralizing gadget attacks while preserving legitimate persisted payloads. The binder is attached at EVERY TypeNameHandling deserialization site — the four above plus the AutoMapper JobProfile.cs job (de)serialization and the SDK CodeGenService.cs legacy-migration readers (ReadOldEntities/ReadOldRelations) — so no deserialization path can instantiate a non-allowlisted type. Regarding the Roslyn analyzers CA2326–CA2330: the binder-aware rules CA2327/CA2329/CA2330 (which flag a TypeNameHandling≠None setting used WITHOUT a SerializationBinder) report zero findings because a fail-closed binder is present at every site; CA2326 (and, depending on how each settings object is constructed, CA2328) still flag ANY TypeNameHandling≠None usage BY DESIGN, independent of the binder. Those two are not un-remediated vulnerabilities but an accepted, documented posture: the platform genuinely requires polymorphic round-tripping of persisted job/notification/entity/relation payloads (so a blanket TypeNameHandling.None is not applicable), and the residual risk is fully mitigated by the fail-closed allowlist — runtime-proven, in that forged $type gadgets throw JsonSerializationException and are never instantiated. The only remaining TypeNameHandling.All usages without a binder are the two serialize-only CodeGenService code-generation template emitters (JsonConvert.SerializeObject into generated .cs source), which deserialize no untrusted input and therefore carry no CWE-502 exposure.
```
...
|-------|-----------|--------|
| Insecure deserialization | Roslyn **CA2326–CA2330** (`TypeNameHandling`) | **0 binder-aware findings (CA2327/CA2329/CA2330)** — every deserialization site (`JobDataService`, `NotificationContext`, `DbEntityRepository`, `DbRelationRepository`, `JobProfile`, and the SDK `CodeGenService` legacy-migration readers) attaches the fail-closed `ErpSerializationBinder`. `CA2326`/`CA2328` flag any `TypeNameHandling`≠`None` **by design** (independent of the binder) — an accepted, documented posture (polymorphic persistence is required; residual risk fully mitigated by the fail-closed allowlist, runtime-proven). No deserialization path instantiates a non-allowlisted type. |
...
| `WebVella.Erp.Plugins.SDK/Services/CodeGenService.cs` | Legacy-migration readers `ReadOldEntities`/`ReadOldRelations` (`TypeNameHandling.Auto` deserialization of `DbEntity`/`DbEntityRelation` from the old-DB `entities`/`entity_relations` JSON) now attach the `ErpSerializationBinder` allowlist, closing the last unprotected deserialization sink (A08/F-1) | ✅ |
