            ...
            string redisInstancePrefix) where T : class
        {
            ...
            return new WebApplicationFactory<T>().WithWebHostBuilder(builder =>
            {
                ...
                builder.ConfigureServices(services =>
                {
                    ...
                    // ==========================================================
                    // JWT Authentication Override (CRITICAL for integration tests)
                    // In .NET minimal hosting, ConfigureAppConfiguration's
                    // AddInMemoryCollection runs AFTER the host configuration
                    // is assembled but the service's Program.cs reads JWT config
                    // inline from builder.Configuration before Build() finalizes.
                    // PostConfigure ensures the test JWT key, issuer, and audience
                    // override whatever the service loaded at startup, guaranteeing
                    // tokens generated by TestDataSeeder are accepted.
                    // ==========================================================
